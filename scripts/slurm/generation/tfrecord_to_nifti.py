...
def to_niftis(tfrecord_path: Path, brain_generator_config: Path, output_path: Path, tar: bool = False):
    """Convert a TFRecord file into NIfTI files.

    Each volume in the TFRecord file is appended by `_{i}_volume.nii.gz` and the labels by `_{i}_labels.nii.gz`.
    ...
    for i, (image, labels) in tqdm(enumerate(iterator), total=NUM_IMAGE_LABEL_PAIRS):
        ...
        save_volume(
            ...
            str(output_path / f"{tfrecord_path.stem}_{i}_volume.nii.gz"),
        )
        save_volume(
            labels,
            brain_generator.aff,
    ...
    if tar:
        with tarfile.open(f"{output_path / tfrecord_path.stem}.tar", "w") as tarf:
            for file in sorted(list(output_path.glob(f"{tfrecord_path.stem}_*.nii.gz"))):
...
if __name__ == "__main__":
    ...
    if opts.input.is_file():
        to_niftis(opts.input, opts.config, opts.output, opts.tar)
    else:
        file_list = sorted(list(opts.input.glob("*.tfrecord")))
        for i, file in tqdm(enumerate(file_list)):
            if i < opts.skip:
                continue
            to_niftis(file, opts.config, opts.output, opts.tar)
